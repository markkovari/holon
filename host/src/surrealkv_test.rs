...
/// `--user root --pass ...`: this suite passes `kv::build` an empty token
/// (a Basic header naming a user an unauthenticated server does not have is
/// REFUSED, not ignored), so it only ever talks to a server that takes
