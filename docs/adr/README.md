...
| [0090](0090-a-lesson-is-about-a-capability-not-a-sentence.md) | A lesson is about a capability, not a sentence — tag knowledge with the interfaces the work touched, and join the two graphs by key | **proposed**: the capability graph is derived and the knowledge pool is history, so they must stay separate stores and meet at query time. Both paid runs so far died on facts about an INTERFACE that were indexed by the wording of a goal |
