...
| [0095](0095-what-is-allowed-to-be-native.md) | What is allowed to be native — the rule the tree already followed | **accepted**. Three questions a thing must answer before it may be a process: does it need a capability WASI denies a guest (not a convenience), is it the smallest such thing, and does it answer a contract a component could have answered — if so the contract stays in WIT and the native side satisfies it over a socket. Names the five that qualified. Explicitly does NOT license a shared Rust library for components: `cargo-component` generates a `bindings.rs` per crate, so the duplication is structural and the control is a lint |
| [0096](0096-a-pull-contract-needs-a-relay.md) | A pull contract needs a relay, and the relay is native — the sixth native thing | **accepted**, and built: `comp-relay`. `sched:timer`, `event:bus` and `cron:expr` are all pull by design, each saying in its own WIT that a relay must drive it — and nothing did, so on k8s it was a curl-loop Deployment and on the native lane a person refreshing a tab. It POSTs `/internal/pump` on a schedule and speaks no WIT, so no app changes to gain a trigger. Push (a watch on the bus seq keys) is an optimisation and never replaces the sweep: core-NATS is at-most-once and no KV change announces the passage of time. Takes a lease for a sharper reason than the reconciler's — two relays draining one consumer group turn at-least-once into at-most-once silently. Measured: a failing saga sat at `running` with no relay and reached `compensated` in seconds with one |
