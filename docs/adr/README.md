...
| [0081](0081-fitness-fuel-and-what-the-swarm-knows.md) | Fitness, fuel, and what the swarm knows — judging a branch, sharing knowledge, and stopping | §2 (knowledge) built as ADR-0084; §1 (fitness) also built — `comp-checks`/`comp-goalrun`, 21 tests — but only 2 of 5 completion states exist; §3 (fuel) unbuilt |
| [0082](0082-a-project-owns-a-repo-and-a-queue.md) | A project owns a repo and a queue — one repo, a dead-letter queue, and a human starts every goal | accepted; the queue and the runner (`comp-goalrun`/`comp-checks`) are both built now |
...
| [0099](0099-an-agent-native-code-store.md) | An agent-native code store: an oplog over a commutative, symbol-level patch graph | **proposed** — contract `wit/vcs/vcs.wit`, crate `crates/holon-vcs` |
