...
ffmpeg -y -ss "$START" -i "$IN" -i "$PAL" -lavfi "fps=$FPS,scale=$WIDTH:-1:flags=lanczos[x];[x][1:v]paletteuse=dither=sierra2_4a" "$OUT" >/dev/null 2>&1
command -v gifsicle >/dev/null && gifsicle -O3 --lossy=25 "$OUT" -o "$OUT" >/dev/null 2>&1 || true
