...
# Probes are reusable but exist to exercise one capability from the outside, so
# they are grouped away from the capabilities a caller is shopping for. The
# `-probe` suffix is the convention; these are the ones it does not catch.
PROBES = {"adversary", "twofile", "bigadd", "demo", "mock-fitness", "mock-provider"}


def slugify(title: str) -> str:
    """GitHub heading anchor: lowercase, drop punctuation, spaces to dashes."""
    keep = "".join(c for c in title.lower() if c.isalnum() or c in " -")
    return keep.replace(" ", "-")
...
def main() -> None:
    ...
    def role(e):
        if e["unimplemented"]:
            return "contract"
        if not e["reusable_as_is"]:
            return "app"
        if e["name"].endswith("-probe") or e["name"] in PROBES:
            return "probe"
        return "capability"
    ...
    GROUPS = [
        (
            ...
            "Each one exports its contract and imports only generic WASI, so it drops "
            "into another app via `wac plug` or a link, configured through "
            "`wasi:config`. This is the part of the tree meant to be reused; "
            "[ADR-0089](../docs/adr/0089-capability-accumulation.md) is why a gate "
            "makes you look here before you build.",
        ),
        (
            "app",
            "Applications and demos — not reusable as-is",
            "A whole app, or a demo of one. Listed because it is in the tree and "
            "because its composition is worth reading, but it is a consumer of the "
            "capabilities above rather than one of them. The `-domain` suffix is the "
            "repo convention; `capsearch` uses this flag so a showcase cannot "
            "outrank the capability it is built from. One file each in "
            "[`docs/apps/`](../docs/apps/README.md).",
        ),
        (
            "contract",
            "Contract only — no implementation behind the WIT",
            "The WIT is real and every export returns an `UNIMPLEMENTED:` marker. "
            "These need a host-side capability a wasm guest cannot have (a syscall, "
            "a socket, a subprocess), so they state what a host must satisfy rather "
            "than satisfying it — see "
            "[ADR-0095](../docs/adr/0095-what-is-allowed-to-be-native.md).",
        ),
        (
            "probe",
            "Probes — test harnesses",
            "Built to exercise one capability from the outside and prove it is "
            "linkable. Reusable, but only useful if you are testing the thing they "
            "probe.",
        ),
    ]
    ...
    for e in entries:
        buckets[role(e)].append(e)

    lines = [
        "# Component catalog",
        "",
        f"Generated by `tools/gen-catalog.py` — do not edit by hand. "
        f"{len(entries)} components.",
        "",
        "Grouped by what a component *is*, because one alphabetical table of "
        f"{len(entries)} rows answered no question anybody actually asks. For what a "
        "component really imports (read out of the built wasm rather than its "
        "source), see [`docs/CAPABILITY-GRAPH.md`](../docs/CAPABILITY-GRAPH.md).",
        "",
    ]
    for key, title, _ in GROUPS:
        lines.append(f"- [{title}](#{slugify(title)}) — {len(buckets[key])}")
    lines.append("- [Descriptions](#descriptions) — all of them, alphabetical")
    lines.append("")
