...
/// A JPEG original: no raw metadata worth the name, and the "green plane" is
/// the G channel, linearised from sRGB.
fn decode_jpeg(path: &Path) -> Result<Decoded> {
    ...
    let metadata = json!({
        ...
        "fnumber": null, "focal_mm": null, "iso": null, "width": w, "height": h,
    });
