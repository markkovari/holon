...
fn a_run_writes_what_it_failed_on_and_a_later_run_reads_it() {
    ...
    let lessons = pool
        .recall(goal, &Reading { k: 3, budget: 1200, pools: vec![] })
    ...
    assert!(matches!(second.stopped, comp_reconciler::generation::SearchStop::Accepted), "{:?}", second.stopped);
    ...
    assert!(
        !pool.recall(goal, &Reading { k: 3, budget: 1200, pools: vec![] }).unwrap().is_empty(),
    ...
    let after = pool
        .recall(goal, &Reading { k: 5, budget: 1200, pools: vec!["patterns".into()] })
        .expect("the pool answered");
