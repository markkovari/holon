...
/// Build the probe with a tag AND a capability list.
fn build_caps(tag: &str, caps: &str) -> Vec<u8> {
    let _guard = BUILD.lock().unwrap_or_else(|e| e.into_inner());
    let components = repo_root().join("components");
    let out = Command::new("cargo")
        .current_dir(&components)
        ...
        .output()
        .expect("running cargo");
    assert!(out.status.success(), "building {tag} failed:\n{}", String::from_utf8_lossy(&out.stderr));
    let path = components.join("target/wasm32-wasip2/release/version_probe.wasm");
    std::fs::read(&path).unwrap_or_else(|e| panic!("reading {}: {e}", path.display()))
}

/// The WHOLE self-report the running version answers with, over the lattice.
fn report(fleet: &Fleet) -> Option<Value> {
    let http = reqwest::blocking::Client::builder().timeout(Duration::from_secs(10)).build().unwrap();
    let r = http
        .get(format!("http://127.0.0.1:{}/", fleet.ingress_port))
        .header("host", "ver.ada.test")
...
/// THE RECURSIVE STEP. A version is deployed, it reports its health and what it
/// can do over the lattice, and then a MORE CAPABLE version is deployed over the
/// top — and the lattice, asked the same question, answers with the larger
/// capability set. Not a record the platform kept: the running code's own report,
/// read over NATS through the ingress.
#[test]
fn a_more_capable_version_reports_itself_over_the_lattice() {
    let base = "diff-writer,temperature-tiers,component-scope";
    let more = "diff-writer,temperature-tiers,component-scope,over-lattice-selfeval";
    let v1 = build_caps("capv1", base);
    let v2 = build_caps("capv2", more);
    assert_ne!(v1, v2, "a more-capable build must be different bytes, or nothing was shipped");

    let fleet = Fleet::start_with_platform("selfeval", 1);
    let api = Api::new(fleet.platform_url());

    // --- deploy v1 and ask it, over the lattice, what it is ------------------
    assert!(matches!(api.upload("ver", v1), 200 | 201), "uploading v1 failed");
    let (code, dep) =
        api.post("/api/deployments", json!({ "name": "ver", "nodes": [{"id": "ver"}], "edges": [] }));
    assert_eq!(code, 201, "deploy failed: {dep}");
    let id = dep["id"].as_str().unwrap().to_string();

    let deadline = Instant::now() + Duration::from_secs(180);
    while Instant::now() < deadline {
        let (c, _) = api.post(&format!("/api/deployments/{id}/save"), json!({}));
        let _ = c;
        if report(&fleet).and_then(|r| r["tag"].as_str().map(str::to_string)).as_deref() == Some("capv1") {
            break;
        }
        std::thread::sleep(Duration::from_secs(2));
    }
    let r1 = report(&fleet).expect("v1 never answered over the lattice");
    assert_eq!(r1["healthy"], true, "v1 reported unhealthy: {r1}");
    let caps1 = r1["capability_count"].as_u64().unwrap();
    println!("    v1 over the lattice: healthy={}, {} capabilities {}", r1["healthy"], caps1, r1["capabilities"]);

    // --- deploy the MORE capable v2 over the top, ask again ------------------
    assert!(matches!(api.upload("ver", v2), 200 | 201), "uploading v2 failed");
    let deadline = Instant::now() + Duration::from_secs(180);
    while Instant::now() < deadline {
        let (c, _) = api.post(&format!("/api/deployments/{id}/save"), json!({}));
        let _ = c;
        if report(&fleet).and_then(|r| r["tag"].as_str().map(str::to_string)).as_deref() == Some("capv2") {
            break;
        }
        std::thread::sleep(Duration::from_secs(2));
    }
    let r2 = report(&fleet).expect("v2 never answered over the lattice");
    assert_eq!(r2["healthy"], true, "the new version reported unhealthy: {r2}");
    let caps2 = r2["capability_count"].as_u64().unwrap();
    println!("    v2 over the lattice: healthy={}, {} capabilities {}", r2["healthy"], caps2, r2["capabilities"]);
