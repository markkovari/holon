...
fn a_reply_is_drafted_queued_and_not_sent_by_this_part() {
    ...
    let Some(gate) = Gate::compose_and_start_with_egress(
        "support", "support-desk-domain", &cfg, &[&shim_egress, &sink_egress],
    ) else {
        return;
    };
    sink.forget();
    ...
    let (_, seeded) = gate.post("/test/seed", None, json!({"target": format!("webhook:{}", sink.url())}));
    let ids: Vec<String> = parse(&seeded)["ticket_ids"]
        .as_array()
        .map(|a| a.iter().filter_map(|v| v.as_str().map(str::to_string)).collect())
        .unwrap_or_default();
    assert!(ids.len() >= 2, "the fixture produced no tickets — the scaffold is broken, not the part");
    let (one, two) = (ids[0].clone(), ids[1].clone());

    let reply = |id: &str| {
        gate.with_headers("POST", &format!("/api/tickets/{id}/reply"), Some(&t),
            &[("x-session", &sid), ("x-csrf", &csrf)], None)
    };
    ...
    let (c, _) = gate.with_headers("POST", &format!("/api/tickets/{one}/reply"), Some(&t),
        &[("x-session", &sid), ("x-csrf", "wrong")], None);
    assert_eq!(c, 403, "a reply with the wrong csrf token must be 403 csrf_invalid");
    let (c, _) = gate.with_headers("POST", &format!("/api/tickets/{one}/reply"), Some(&t),
        &[("x-session", "nope"), ("x-csrf", &csrf)], None);
    assert_eq!(c, 403, "a reply against a session that does not exist must be 403 session_expired");
    let (c, _) = gate.with_headers("POST", &format!("/api/tickets/{one}/reply"), None,
        &[("x-session", &sid), ("x-csrf", &csrf)], None);
    ...
    assert!(!r.trim().is_empty(), "the route answered an empty body — it is not implemented, or it trapped");
    let d = parse(&r);
    assert!(d["event"].as_str().is_some_and(|s| !s.is_empty()), "the answer must name the outbox event the reply is waiting in: {d}");
    ...
    assert!(rep["event"].as_str().is_some_and(|s| !s.is_empty()), "the stored reply must name its outbox event: {rep}");
    assert!(rep["drafted_at"].as_str().unwrap_or_default().ends_with('Z'), "drafted_at must be RFC3339 UTC: {rep}");
