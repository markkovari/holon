...
fn tickets_open_validate_their_address_and_list_oldest_first() {
    ...
    let (c, _) = gate.post("/api/tickets", Some(&w),
    ...
    for bad in ["ada@example.test", "mailto:ada@example.test", "https://acme.test/hooks/ada", ""] {
        let (c, _) = gate.post("/api/tickets", Some(&w), json!({"subject":"s","body":"b","customer":bad}));
        assert_eq!(c, 400, "customer '{bad}' cannot be delivered to and must be 400 invalid_ticket");
    }
    ...
    assert!(d.get("reply").is_none(), "tickets must not invent a reply — that is the reply part's job");
    assert!(d["opened_at"].as_str().unwrap_or_default().ends_with('Z'), "opened_at must be RFC3339 UTC: {d}");

    let (_, read) = gate.get(&format!("/api/tickets/{id}"), Some(&w));
    assert!(!read.trim().is_empty(), "the route answered an empty body — it is not implemented, or it trapped");
    let d = parse(&read);
    ...
    assert_eq!(answered["tickets"], json!([]), "nothing is answered yet and the list said otherwise");
}
