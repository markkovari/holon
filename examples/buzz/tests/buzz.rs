...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e buzz`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose buzz)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=buzz"])
        .spawn()
