...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e pulse`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose pulse)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=pulse"])
        .spawn()
