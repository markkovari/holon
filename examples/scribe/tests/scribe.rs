...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e scribe`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose scribe)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=scribe"])
        .spawn()
