...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e stash`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose stash)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=stash"])
        .spawn()
