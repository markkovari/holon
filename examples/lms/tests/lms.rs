...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e lms`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose lms)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=lms"])
        .spawn()
