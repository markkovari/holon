//! E2E for smart-home: household devices behind a login, with an on/off toggle.
//!
//! Self-contained: spawns comp-host on the composed artifact apps/smart-home.toml names,
//! on a free port, with in-memory kv. Run with `cargo xtask e2e smart-home`.
...
/// Spawn comp-host on the composed artifact `apps/smart-home.toml` names.
fn start_host(extra: &[&str]) -> HostGuard {
    let bin = root().join("host/target/release/comp-host");
    let component = root().join("components/target/smart-home.composed.wasm");
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e smart-home`)");
    assert!(component.exists(), "composed wasm missing: {component:?} (run `cargo xtask compose smart-home`)");
    let addr = format!("127.0.0.1:{}", free_port());
    let child = Command::new(&bin)
        .args(["--component", component.to_str().unwrap(), "--addr", &addr, "--kv", "memory"])
        .args(["--app", "smart-home", "--tenant", "smart-home"])
        .args(["--config-file", concat!(env!("CARGO_MANIFEST_DIR"), "/../defaults.conf")])
        .args(["--config", "default-tenant=smart-home"])
        .args(extra)
        .spawn()
...
/// Devices are owner-scoped; toggle flips `state` off -> on -> off and is
/// refused for someone else's device.
#[test]
fn e2e() {
    let host = start_host(&[]);
    let base = host.1.as_str();
    auth_surface(base, "Smart Home");

    let (s, _) = req(base, "GET", "/styles.css", None, None);
    assert_eq!(s, 200, "the UI's stylesheet is served");

    let (alice, a) = account(base, "alice@example.com");
    let (_, b) = account(base, "bob@example.com");

    let (s, _) = jreq(base, "POST", "/api/items", None, Some(json!({ "name": "lamp" })));
    assert_eq!(s, 401);

    let (s, d) = jreq(base, "POST", "/api/items", Some(&a), Some(json!({ "name": "lamp" })));
    assert_eq!(s, 201, "create device: {d}");
    assert_eq!(d["state"], json!("off"));
    assert_eq!(d["owner"], json!(alice));
    let id = d["id"].as_str().expect("id").to_string();

    let (s, d) = jreq(base, "POST", &format!("/api/items/{id}/toggle"), Some(&a), None);
    assert_eq!(s, 200, "toggle: {d}");
    assert_eq!(d["state"], json!("on"));

    let (s, _) = jreq(base, "POST", &format!("/api/items/{id}/toggle"), Some(&b), None);
    assert_eq!(s, 403, "bob cannot toggle alice's device");

    let (_, d) = jreq(base, "POST", &format!("/api/items/{id}/toggle"), Some(&a), None);
    assert_eq!(d["state"], json!("off"), "a second toggle turns it back off");

    let (s, _) = jreq(base, "POST", "/api/items/does-not-exist/toggle", Some(&a), None);
    assert_eq!(s, 404);

    let (_, list) = jreq(base, "GET", "/api/items", Some(&a), None);
    let items = list["items"].as_array().unwrap();
    assert_eq!(items.len(), 1, "{list}");
    assert_eq!(items[0]["state"], json!("off"), "the toggles were persisted");

    let (_, list) = jreq(base, "GET", "/api/items", Some(&b), None);
    assert_eq!(list["items"], json!([]), "bob sees none of alice's devices");
}
