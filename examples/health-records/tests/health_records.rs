//! E2E for health-records: patient records behind a login; create is gated on the `doctor` role.
//!
//! Self-contained: spawns comp-host on the composed artifact apps/health-records.toml names,
//! on a free port, with in-memory kv. Run with `cargo xtask e2e health-records`.
...
/// Spawn comp-host on the composed artifact `apps/health-records.toml` names.
fn start_host(extra: &[&str]) -> HostGuard {
    let bin = root().join("host/target/release/comp-host");
    let component = root().join("components/target/health-records.composed.wasm");
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e health-records`)");
    assert!(component.exists(), "composed wasm missing: {component:?} (run `cargo xtask compose health-records`)");
    let addr = format!("127.0.0.1:{}", free_port());
    let child = Command::new(&bin)
        .args(["--component", component.to_str().unwrap(), "--addr", &addr, "--kv", "memory"])
        .args(["--app", "health-records", "--tenant", "health-records"])
        .args(["--config-file", concat!(env!("CARGO_MANIFEST_DIR"), "/../defaults.conf")])
        .args(["--config", "default-tenant=health-records"])
        .args(extra)
        .spawn()
...
/// Items are role-gated (`doctor`) on create and owner-scoped on read. A fresh
...
fn e2e() {
    ...
    auth_surface(base, "health-records");
    ...
    assert_eq!(s, 403, "create without the doctor role is forbidden: {body}");
