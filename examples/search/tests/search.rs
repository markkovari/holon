...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e search`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose search)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=search"])
        .spawn()
