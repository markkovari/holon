//! E2E for freight-tracker: shipments behind a login; create is gated on the `dispatcher` role.
//!
//! Self-contained: spawns comp-host on the composed artifact apps/freight-tracker.toml names,
//! on a free port, with in-memory kv. Run with `cargo xtask e2e freight-tracker`.
...
/// Spawn comp-host on the composed artifact `apps/freight-tracker.toml` names.
fn start_host(extra: &[&str]) -> HostGuard {
    let bin = root().join("host/target/release/comp-host");
    let component = root().join("components/target/freight-tracker.composed.wasm");
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e freight-tracker`)");
    assert!(component.exists(), "composed wasm missing: {component:?} (run `cargo xtask compose freight-tracker`)");
    let addr = format!("127.0.0.1:{}", free_port());
    let child = Command::new(&bin)
        .args(["--component", component.to_str().unwrap(), "--addr", &addr, "--kv", "memory"])
        .args(["--app", "freight-tracker", "--tenant", "freight-tracker"])
        .args(["--config-file", concat!(env!("CARGO_MANIFEST_DIR"), "/../defaults.conf")])
        .args(["--config", "default-tenant=freight-tracker"])
        .args(extra)
        .spawn()
...
/// Items are role-gated (`dispatcher`) on create and owner-scoped on read. A fresh
...
fn e2e() {
    ...
    auth_surface(base, "freight-tracker");
    ...
    assert_eq!(s, 403, "create without the dispatcher role is forbidden: {body}");
