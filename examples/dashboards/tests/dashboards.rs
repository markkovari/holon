...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e dashboards`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose dashboards)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=dashboards"])
        .spawn()
