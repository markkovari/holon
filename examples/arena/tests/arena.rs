...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e arena`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose arena)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=arena"])
        .spawn()
