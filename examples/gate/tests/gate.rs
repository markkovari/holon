...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e gate`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose gate)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=gate"])
        .spawn()
