...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e books`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose books)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=books"])
        .spawn()
