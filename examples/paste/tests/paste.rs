...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e paste`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose paste)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=paste"])
        .spawn()
