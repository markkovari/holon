...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e abtest`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose abtest)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=abtest"])
        .spawn()
