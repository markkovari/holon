...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e booked`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose booked)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=booked"])
        .spawn()
