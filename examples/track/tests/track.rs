...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e track`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose track)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=track"])
        .spawn()
