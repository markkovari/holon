...
fn start_host() -> HostGuard {
    ...
    assert!(bin.exists(), "host not built: {bin:?} (run `cargo xtask e2e tempo`)");
    assert!(component.exists(), "composed wasm missing (cargo xtask compose tempo)");
    let child = Command::new(&bin)
        ...
        .args(["--config", "default-tenant=tempo"])
        .spawn()
