...
fn ensure_fsm() {
    let def = fsm::Definition {
        states: vec!["pending".to_string(), "settled".to_string(), "refused".to_string(), "compensated".to_string()],
        initial: "pending".to_string(),
        transitions: vec![
            fsm::Transition { event: "settle".to_string(), source: "pending".to_string(), target: "settled".to_string() },
            fsm::Transition { event: "refuse".to_string(), source: "pending".to_string(), target: "refused".to_string() },
            fsm::Transition { event: "compensate".to_string(), source: "settled".to_string(), target: "compensated".to_string() },
        ],
...
fn write_transfer(route: &Route, body: &str) -> Reply {
        ...
        match money::subtract(&cur_amount, &amount) {
            Ok(rem) => {
                if rem.units < 0 {
                    ...
                    let _ = records::create("transfers", &transfer_doc.to_string(), &["state".to_string()]);
    ...
    let t_entry = match records::create("transfers", &transfer_doc.to_string(), &["state".to_string()]) {
        Ok(e) => e,
        Err(_) => {
    ...
    let ledger_entry = ledger::Entry {
        ...
        lines: vec![
            ledger::Line { account: from_id.to_string(), amount: amount.units, side: ledger::Side::Debit },
            ledger::Line { account: to_id.to_string(), amount: amount.units, side: ledger::Side::Credit },
        ],
    };
    ...
    let j_entry = match records::create("journal", &j_doc.to_string(), &["from".to_string(), "to".to_string()]) {
        Ok(e) => e,
