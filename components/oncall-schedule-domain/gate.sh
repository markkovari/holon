#!/usr/bin/env bash
# oncall-schedule-domain's held-out-spec gate: compose the real capability graph, host it, and
# assert real HTTP behavior — not just that `cargo component check` is happy.
set -uo pipefail
cd "$(dirname "$0")/../.." || exit 1

DOMAIN="oncall-schedule-domain"
PORT="${GATE_PORT:-3906}"
...
"$HOST" --app "$DOMAIN" --component "$ART" --addr "127.0.0.1:$PORT" --kv sqlite \
  --config "default-tenant=oncallschedule" >/tmp/gate-$DOMAIN-host.log 2>&1 &
HOSTPID=$!
...
python3 components/domain-gate.py "http://127.0.0.1:$PORT" '{"lead": ["shifts:write", "shifts:read", "shifts:reassign"], "engineer": ["shifts:write", "shifts:read", "shifts:reassign"]}' \
