#!/usr/bin/env bash
# ats-domain's held-out-spec gate: compose the real capability graph, host it, and
...
"$HOST" --app "$DOMAIN" --component "$ART" --addr "127.0.0.1:$PORT" --kv sqlite \
  --static-dir examples/ats/public \
  --config "default-tenant=ats" >/tmp/gate-$DOMAIN-host.log 2>&1 &
HOSTPID=$!
