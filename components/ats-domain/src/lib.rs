...
const ROLES: &[&str] = &["admin", "interviewer"];
...
/// A verified caller for a domain route. `require_admin` reads `roles`
/// directly (the same style `track-domain` uses) rather than going through
/// `auth:identity/rbac`'s permission-mapping layer — this app has exactly two
/// roles and one admin/rep split, which a role check says as plainly as a
/// permission check would, with no `rbac::set-role-permissions` seeding step
/// to keep in sync.
...
fn register(body: &str) -> Reply {
    ...
    let wanted = req.role.unwrap_or_else(|| "interviewer".to_string());
    let role = if ROLES.contains(&wanted.as_str()) { wanted } else { "interviewer".to_string() };
