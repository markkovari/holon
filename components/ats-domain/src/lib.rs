//! `ats-domain` — router: auth (register/login/logout/me) and dispatch into
//! `handlers.rs` for the domain routes. The whole router is
//! `guestauth`/`guestio` macro expansions; nothing domain-specific lives
//! here — see `handlers.rs` for the actual ATS logic.
...
/// back to `interviewer` — a role is a privilege, not a free-text field.
...
guestauth::guest_accounts_endpoints!(TENANT, ROLES, "interviewer");
