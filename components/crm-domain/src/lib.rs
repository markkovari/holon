...
fn register(body: &str) -> Reply {
    ...
    let wanted = req.role.unwrap_or_else(|| "rep".to_string());
    let role = if ROLES.contains(&wanted.as_str()) { wanted } else { "rep".to_string() };
