...
// Embedded React SPA assets (generated by build.rs from static/). The component
// serves the UI itself — no static-file provider, no Node. STATIC_ASSETS is
// `&[(web_path, content_type, bytes)]`.
mod static_assets {
...
// ---- static SPA (embedded) ----------------------------------------------
// Serve the built React app from the wasm itself: exact path match, else the
// SPA fallback to index.html (client-side routing). API paths never reach here
// (they matched earlier in the router).

fn serve_static(path: &str) -> Outcome {
    let want = if path == "/" { "/index.html" } else { path };
    for (p, ct, bytes) in static_assets::STATIC_ASSETS {
        if *p == want {
            return Outcome::Raw(200, (*ct).to_string(), bytes.to_vec());
        }
    }
