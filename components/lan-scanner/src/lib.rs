//! `lan-scanner` — a WIT contract with NO implementation behind it.
//!
...
//! in. It cannot be filled in from here: to scan a LAN needs raw sockets on the host network,
...
struct Component;
impl Guest for Component { fn scan() -> String { "UNIMPLEMENTED: lan-scanner cannot scan a LAN from wasm".to_string() } }
