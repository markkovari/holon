...
fn create_item(request: &IncomingRequest) -> Outcome {
    ...
    if !p.roles.contains(&"agent".to_string()) {
    ...
    match records::create("transactions", &d.to_string(), &["owner".to_string()]) {
        Ok(rec) => {
...
fn list_items(request: &IncomingRequest) -> Outcome {
    ...
    let items: Vec<Value> =
        records::find_by("transactions", "owner", &json!(p.subject).to_string())
            .unwrap_or_default()
            .iter()
            .filter_map(|e| {
