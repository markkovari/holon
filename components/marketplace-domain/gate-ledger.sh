#!/usr/bin/env bash
# marketplace-domain's PART gate — ledger.rs alone, judged against
# scenario-ledger.hurl. Uses /test/seed and the raw /test/* read routes so
...
"$HOST" --app "$DOMAIN" --component "$ART" --addr "127.0.0.1:$PORT" --kv sqlite \
  --config "default-tenant=marketplace" >/tmp/gate-$DOMAIN-host-ledger.log 2>&1 &
HOSTPID=$!
...
hurl --variable "base=http://127.0.0.1:$PORT" "components/$DOMAIN/scenario-ledger.hurl"
STATUS=$?
if [ $STATUS -ne 0 ]; then
  echo "gate FAILED — host log:"; tail -n 60 /tmp/gate-$DOMAIN-host-ledger.log
fi
exit $STATUS
