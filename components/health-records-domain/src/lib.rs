...
const TENANT: &str = "health-records";
const DOMAIN: &str = "Electronic Health Records";
...
<body>
    <h1>health-records (Electronic Health Records)</h1>
...
fn create_item(request: &IncomingRequest) -> Outcome {
    ...
    if !p.roles.contains(&"doctor".to_string()) {
    ...
    match records::create("patients", &d.to_string(), &["owner".to_string()]) {
        Ok(rec) => {
...
fn list_items(request: &IncomingRequest) -> Outcome {
    ...
    let mut items: Vec<Value> = records::find_by("patients", "owner", &json!(p.subject).to_string())
        .unwrap_or_default()
        .iter()
