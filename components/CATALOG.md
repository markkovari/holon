...
Generated by `tools/gen-catalog.py` — do not edit by hand. 204 components.

Grouped by what a component *is*, because one alphabetical table of 204 rows answered no question anybody actually asks. For what a component really imports (read out of the built wasm rather than its source), see [`docs/CAPABILITY-GRAPH.md`](../docs/CAPABILITY-GRAPH.md).
...
| **binder-domain** | `binder:app@0.1.0` | auth:identity/accounts, auth:identity/authorizer, auth:identity/types, card:identify/identifier, deck:build/builder, portfolio:value/valuation, price:history/history, kv:store | — | 296 KiB |
